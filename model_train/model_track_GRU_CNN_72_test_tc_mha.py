...
for i in range(len(i_index)):
    ...
    essemble_values=essemble_values.reshape(essemble_values.shape[0],17,4)
    essemble_values_new=essemble_values[:,:,:]
    essemble_values_new=essemble_values_new.reshape(essemble_values_new.shape[0],68)
...
data_y_track=data_y.reshape((data_y.shape[0], 12, 4))
data_track=data_y_track[:,11:12,0:4]
...
drop_x=pd.read_csv(f"./drop_idx_72.txt",names=['1'])
...
uv_label_ds=[]
# 48 1336
glob_path=glob.glob('../compose_slp/compose_slp_npy_72_usa_2010_2024/*')
print(len(glob_path))
slp_label_latlon_ds=[]
for idx in range(len(j_names)):
    j = j_names[idx]
    i = i_names[idx]
    slp_label_72=np.load(f"../compose_slp/compose_slp_npy_72_usa_2010_2024/{str(i)}_{str(j)}_slp.npy")
    slp_label_latlon=np.load(f"../compose_slp/compose_slp_latlon_npy_usa_2010_2024_72/{str(i)}_{str(j)}_slp_latlon.npy")
    slp_label=slp_label_72
...
slp_tc_24=pd.read_csv(f"./slp_2010_2024_72.csv",index_col=0,header=0)
slp_tc=slp_tc_24.values

y_scaler2 = MinMaxScaler(feature_range=(0, 1)).fit(slp_tc)
slp_tc_new= y_scaler2.transform(slp_tc)
slp_tc_new_xin0=slp_tc_new.reshape(slp_tc_new.shape[0],12,7)
slp_tc_new_xin1=slp_tc_new_xin0[:,:,0:5]
slp_tc_new_xin2=slp_tc_new_xin1.reshape(slp_tc_new_xin1.shape[0],60)
...
fore_input = Input(shape=60, name='fore_input')
...
out_put=Dense(4,activation='sigmoid')(out)
...
model.summary()
# history = model.fit([train_X,fore_tc_train],train_track,validation_data=([valid_X,fore_tc_valid],valid_track) ,epochs=60,batch_size=16, callbacks=[es,reduce_lr],verbose=2,workers=5)
...
model.load_weights('./GRU_CNN_72_mha.hdf5')
tf.compat.v1.enable_eager_execution()
yhat=model([valid_P_label,valid_X,fore_tc_valid])
print(len(lat0),len(lon0))
