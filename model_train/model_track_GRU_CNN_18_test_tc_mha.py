...
for i in range(len(i_index)):
    ...
    essemble=shift(dataframe_track,18)
    essemble_values=essemble.values
    essemble_values=essemble_values.reshape(essemble_values.shape[0],8,4)
    essemble_values_new=essemble_values[:,:,:]
    essemble_values_new=essemble_values_new.reshape(essemble_values_new.shape[0],32)
    essemble_list=essemble_values_new.tolist()
    dif_data=dis_direct(essemble_list,18)
...
data_y_track=data_y.reshape((data_y.shape[0], 3, 4))
data_track=data_y_track[:,2:3,0:4]
...
drop_x=pd.read_csv(f"./drop_idx_18.txt",names=['1'])
...
uv_label_ds=[]
# 48 1336
glob_path=glob.glob('.../compose_slp/compose_slp_npy_18_usa_2010_2024/*')
print(len(glob_path))
slp_label_latlon_ds=[]
for idx in range(len(j_names)):
    j = j_names[idx]
    i = i_names[idx]
    slp_label=np.load(f"../compose_slp/compose_slp_npy_18_usa_2010_2024/{str(i)}_{str(j)}_slp.npy")
    slp_label_latlon=np.load(f"../compose_slp/compose_slp_latlon_npy_usa_2010_2024_18/{str(i)}_{str(j)}_slp_latlon.npy")
...
slp_tc_24=pd.read_csv(f"./slp_2010_2024_18.csv",index_col=0,header=0)
slp_tc=slp_tc_24.values
print(len(slp_tc))
y_scaler2 = MinMaxScaler(feature_range=(0, 1)).fit(slp_tc)
slp_tc_new= y_scaler2.transform(slp_tc)
slp_tc_new_xin0=slp_tc_new.reshape(slp_tc_new.shape[0],3,7)
slp_tc_new_xin1=slp_tc_new_xin0[:,:,0:5]
slp_tc_new_xin2=slp_tc_new_xin1.reshape(slp_tc_new_xin1.shape[0],15)
...
slp=Dense(32,activation='relu')(slp)
...
encoder_outputs1=Dense(24,activation='relu')(encoder_outputs1)


fore_input = Input(shape=15, name='fore_input')
fore_output=Dense(128,activation='relu')(fore_input)
fore_output=Dense(64,activation='sigmoid')(fore_output)
out_tracks=tf.concat([encoder_outputs1,fore_output],axis=-1)
out_tracks=Dense(32,activation='sigmoid')(out_tracks)

# out_tracks=tf.concat([slp,encoder_outputs1,fore_output],axis=-1)

query=Reshape((32, 1))(slp)
key=Reshape((1, 32))(out_tracks)
query_new=tf.repeat(query, repeats=[32], axis=2)
key_new=tf.repeat(key, repeats=[32], axis=1)
value=tf.add(query_new,key_new)
score = tf.matmul(query, key)
scaled_score = score / tf.math.sqrt(tf.cast(64,score.dtype))
# scaled_score=Reshape((32*32,))(scaled_score)
# weights = tf.nn.softmax(scaled_score, axis=-1)
# weights=Reshape((32,32))(weights)
weights=scaled_score
y = tf.multiply(weights, value)
out=Flatten()(y)

# out_put=Dense(32,activation='relu')(out)
out_put=Dense(4,activation='relu')(out)
# model = Model(inputs=[fore_input], outputs=out_put)
...
model.load_weights('./GRU_CNN_18_mha.hdf5')
