...
for i in range(len(i_index)):
    ...
    essemble=shift(dataframe_track,36)
    essemble_values=essemble.values
    essemble_values=essemble_values.reshape(essemble_values.shape[0],11,4)
    essemble_values_new=essemble_values[:,:,:]
    essemble_values_new=essemble_values_new.reshape(essemble_values_new.shape[0],44)
    essemble_list=essemble_values_new.tolist()
    dif_data=dis_direct(essemble_list,36)
...
data_y_track=data_y.reshape((data_y.shape[0], 6, 4))
data_track=data_y_track[:,5:6,0:4]
...
drop_x=pd.read_csv(f"./drop_idx_36.txt",names=['1'])
...
uv_label_ds=[]
# 48 1336
glob_path=glob.glob('.../compose_slp/compose_slp_npy_36_usa_2010_2024/*')
print(len(glob_path))
slp_label_latlon_ds=[]
for idx in range(len(j_names)):
    j = j_names[idx]
    i = i_names[idx]
    slp_label=np.load(f"../compose_slp/compose_slp_npy_36_usa_2010_2024/{str(i)}_{str(j)}_slp.npy")
    slp_label_latlon=np.load(f"../compose_slp/compose_slp_latlon_npy_usa_2010_2024_36/{str(i)}_{str(j)}_slp_latlon.npy")
...
slp_tc_24=pd.read_csv(f"./slp_2010_2024_36.csv",index_col=0,header=0)
slp_tc=slp_tc_24.values
print(len(slp_tc))
y_scaler2 = MinMaxScaler(feature_range=(0, 1)).fit(slp_tc)
slp_tc_new= y_scaler2.transform(slp_tc)
slp_tc_new_xin0=slp_tc_new.reshape(slp_tc_new.shape[0],6,7)
slp_tc_new_xin1=slp_tc_new_xin0[:,:,0:5]
slp_tc_new_xin2=slp_tc_new_xin1.reshape(slp_tc_new_xin1.shape[0],30)
...
fore_input = Input(shape=30, name='fore_input')
...
model.load_weights('./GRU_CNN_36_mha.hdf5')
